"""This module contains the SQLAlchemyAuthUnitOfWorkAdapter class."""
...
class SQLAlchemyUserUnitOfWorkAdapter(UserUnitOfWorkPort):
    """SQLAlchemy-backed Unit of Work for the auth module.

    Creates a fresh :class:`AsyncSession` on entry and exposes the
    ``users`` repository bound to that session. On exit it rolls back
    automatically when an unhandled exception escapes the ``async with``
    block; otherwise callers must explicitly call ``commit()``.

    Usage::

        async with SQLAlchemyUserUnitOfWorkAdapter(session_factory, logger_factory) as uow:
