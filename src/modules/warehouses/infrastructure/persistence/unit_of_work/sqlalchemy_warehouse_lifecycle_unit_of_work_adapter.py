"""This module contains the SQLAlchemyWarehouseLifecycleUnitOfWorkAdapter class."""
...
class SQLAlchemyWarehouseLifecycleUnitOfWorkAdapter(WarehouseLifecycleUnitOfWorkPort):
    """SQLAlchemy-backed Unit of Work for the warehouses and orders query modules.

    Creates a fresh :class:`AsyncSession` on entry and exposes the
    ``warehouses`` and ``orders`` repository bound to that session.
    On exit it rolls back automatically when an unhandled exception
    escapes the ``async with`` block; otherwise callers must
    explicitly call ``commit()``.

    Usage::

        async with SQLAlchemyWarehouseLifecycleUnitOfWorkAdapter(session_factory, logger_factory) as uow:
    ...
    async def __aenter__(self) -> "SQLAlchemyWarehouseLifecycleUnitOfWorkAdapter":
        """Open a new database session and initialise all repositories.

        Returns:
            SQLAlchemyWarehouseLifecycleUnitOfWorkAdapter: This instance, ready to use.
        """
        self._logger.debug("warehouse lifecycle unit of work: begin transaction")
        self._session = self._session_factory()
        self.warehouses = SQLAlchemyWarehouseRepositoryAdapter(
            session=self._session,
            logger_factory_outbound=self._logger_factory,
        )
        self.orders_query = SQLAlchemyWarehouseOrderQueryRepositoryAdapter(
            session=self._session,
            logger_factory_outbound=self._logger_factory,
        )
        return self
    ...
    async def __aexit__(
        ...
        if exc_type is not None:
            self._logger.warning(
                "warehouse lifecycle unit of work: unhandled exception — rolling back",
        ...
        if self._session is not None:
            ...
            self._logger.debug("warehouse lifecycle unit of work: session closed")
    ...
    async def commit(self) -> None:
        ...
        if self._session is None:
            raise RuntimeError(
                "SQLAlchemyWarehouseLifecycleUnitOfWorkAdapter.commit() called outside "
                "of an 'async with' block."
            )
        self._logger.debug("warehouse lifecycle unit of work: commit")
    ...
    async def rollback(self) -> None:
        ...
        if self._session is None:
            raise RuntimeError(
                "SQLAlchemyWarehouseLifecycleUnitOfWorkAdapter.rollback() called outside "
                "of an 'async with' block."
            )
        self._logger.debug("warehouse lifecycle unit of work: rollback")
