"""This module contains the SQLAlchemyProductLifecycleUnitOfWorkAdapter class."""
...
class SQLAlchemyProductLifecycleUnitOfWorkAdapter(ProductLifecycleUnitOfWorkPort):
    """SQLAlchemy-backed Unit of Work for the products and orders query module.

    Creates a fresh :class:`AsyncSession` on entry and exposes the
    ``products`` repository bound to that session. On exit it rolls back
    automatically when an unhandled exception escapes the ``async with``
    block; otherwise callers must explicitly call ``commit()``.

    Usage::

        async with SQLAlchemyProductLifecycleUnitOfWorkAdapter(session_factory, logger_factory) as uow:
    ...
    async def __aenter__(self) -> "SQLAlchemyProductLifecycleUnitOfWorkAdapter":
        """Open a new database session and initialise all repositories.

        Returns:
            SQLAlchemyProductLifecycleUnitOfWorkAdapter: This instance, ready to use.
        """
        self._logger.debug("product lifecycle unit of work: begin transaction")
        self._session = self._session_factory()
        self.products = SQLAlchemyProductRepositoryAdapter(
            session=self._session,
            logger_factory_outbound=self._logger_factory,
        )
        self.orders_query = SQLAlchemyProductOrderQueryRepositoryAdapter(
            session=self._session,
            logger_factory_outbound=self._logger_factory,
        )
        return self
    ...
    async def __aexit__(
        ...
        if exc_type is not None:
            self._logger.warning(
                "product lifecycle unit of work: unhandled exception — rolling back",
        ...
        if self._session is not None:
            ...
            self._logger.debug("product lifecycle unit of work: session closed")
    ...
    async def commit(self) -> None:
        ...
        if self._session is None:
            raise RuntimeError(
                "SQLAlchemyProductLifecycleUnitOfWorkAdapter.commit() called outside "
                "of an 'async with' block."
            )
        self._logger.debug("product lifecycle unit of work: commit")
    ...
    async def rollback(self) -> None:
        ...
        if self._session is None:
            raise RuntimeError(
                "SQLAlchemyProductLifecycleUnitOfWorkAdapter.rollback() called outside "
                "of an 'async with' block."
            )
        self._logger.debug("Product lifecycle unit of work: rollback")
