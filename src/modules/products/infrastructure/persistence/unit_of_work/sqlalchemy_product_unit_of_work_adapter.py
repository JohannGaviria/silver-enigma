"""This module contains the SQLAlchemyProductUnitOfWorkAdapter class."""
...
class SQLAlchemyProductUnitOfWorkAdapter(ProductUnitOfWorkPort):
    """SQLAlchemy-backed Unit of Work for the products module.
    ...
    Usage::
        ...
        async with SQLAlchemyProductUnitOfWorkAdapter(session_factory, logger_factory) as uow:
    ...
    async def __aenter__(self) -> "SQLAlchemyProductUnitOfWorkAdapter":
        """Open a new database session and initialise all repositories.

        Returns:
            SQLAlchemyProductUnitOfWorkAdapter: This instance, ready to use.
        """
        self._logger.debug("product unit of work: begin transaction")
    ...
    async def __aexit__(
        ...
        if exc_type is not None:
            self._logger.warning(
                "product unit of work: unhandled exception — rolling back",
        ...
        if self._session is not None:
            ...
            self._logger.debug("product unit of work: session closed")
    ...
    async def commit(self) -> None:
        ...
        if self._session is None:
            raise RuntimeError(
                "SQLAlchemyProductUnitOfWorkAdapter.commit() called outside "
                "of an 'async with' block."
            )
        self._logger.debug("product unit of work: commit")
    ...
    async def rollback(self) -> None:
        ...
        if self._session is None:
            raise RuntimeError(
                "SQLAlchemyProductUnitOfWorkAdapter.rollback() called outside "
                "of an 'async with' block."
            )
        self._logger.debug("Product unit of work: rollback")
