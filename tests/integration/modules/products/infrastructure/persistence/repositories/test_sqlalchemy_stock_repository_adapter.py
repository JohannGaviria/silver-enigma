...
class TestSQLAlchemyStockRepositoryAdapter:
    ...
    # --------------------------------------------------------------------------
    # find_by_product_and_warehouse_for_update
    # --------------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_should_find_stock_by_product_and_warehouse_for_update(
        self,
        faker: Faker,
        stock_repository: SQLAlchemyStockRepositoryAdapter,
    ) -> None:
        """find_by_product_and_warehouse_for_update() must return the matching stock."""
        product_id = UUID(faker.uuid4())
        warehouse_id = UUID(faker.uuid4())
        ...
        result = await stock_repository.find_by_product_and_warehouse_for_update(
            product_id,
            warehouse_id,
        )
    ...
    async def test_should_return_none_when_stock_for_update_not_found(
        ...
        """find_by_product_and_warehouse_for_update() must return None when stock does not exist."""
        result = await stock_repository.find_by_product_and_warehouse_for_update(
            UUID(faker.uuid4()),
            UUID(faker.uuid4()),
        )
    ...
    @pytest.mark.asyncio
    async def test_should_issue_select_for_update_in_find_by_product_and_warehouse_for_update(
        self,
        faker: Faker,
        stock_repository: SQLAlchemyStockRepositoryAdapter,
    ) -> None:
        """find_by_product_and_warehouse_for_update() must emit SELECT FOR UPDATE."""
        with patch.object(
            stock_repository.session,
            "execute",
            new=AsyncMock(
            ...
            await stock_repository.find_by_product_and_warehouse_for_update(
                UUID(faker.uuid4()),
                UUID(faker.uuid4()),
            )
    ...
    @pytest.mark.asyncio
    async def test_should_not_issue_select_for_update_in_find_by_product_and_warehouse(
        self,
        faker: Faker,
        stock_repository: SQLAlchemyStockRepositoryAdapter,
    ) -> None:
        """find_by_product_and_warehouse() must NOT emit SELECT FOR UPDATE."""
        with patch.object(
            stock_repository.session,
            "execute",
            new=AsyncMock(
