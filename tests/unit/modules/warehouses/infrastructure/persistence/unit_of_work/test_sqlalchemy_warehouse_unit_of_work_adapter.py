...
class TestSQLAlchemyWarehouseUnitOfWorkAdapter:
    ...
    async def test_aenter_returns_self(
        self,
        warehouse_uow_with_session_mock: tuple[
            SQLAlchemyWarehouseUnitOfWorkAdapter, AsyncMock
        ],
    ) -> None:
        """``async with uow`` must return the UoW instance itself."""
        uow, _ = warehouse_uow_with_session_mock
    ...
    @pytest.mark.asyncio
    async def test_aenter_exposes_warehouses_repository(
        self,
        warehouse_uow_with_session_mock: tuple[
            SQLAlchemyWarehouseUnitOfWorkAdapter, AsyncMock
        ],
    ) -> None:
        """``uow.warehouses`` must be a ``SQLAlchemyWarehouseRepositoryAdapter`` after entry."""
        uow, _ = warehouse_uow_with_session_mock

        async with uow:
            assert isinstance(uow.warehouses, SQLAlchemyWarehouseRepositoryAdapter)
    ...
    async def test_aenter_binds_repository_to_current_session(
        ...
        """The ``warehouses`` repository must use the session opened during entry."""
        uow, session_mock = warehouse_uow_with_session_mock

        async with uow:
            assert isinstance(uow.warehouses, SQLAlchemyWarehouseRepositoryAdapter)
            assert uow.warehouses.session is session_mock
    ...
    async def test_each_context_entry_creates_a_new_session(
        ...
        async with uow:
            assert isinstance(uow.warehouses, SQLAlchemyWarehouseRepositoryAdapter)
            first_session = uow.warehouses.session

        async with uow:
            assert isinstance(uow.warehouses, SQLAlchemyWarehouseRepositoryAdapter)
            second_session = uow.warehouses.session
